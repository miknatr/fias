...
class AddressStorageTest extends TestAbstract
{
    ...
    public function testFindAddress()
    {
        $this->assertEquals(
            ...
            $this->storage->findAddress('г москва')
        );

        $this->assertEquals(
            '77303f7c-452b-4e73-b2b0-cbc59fe636c2',
            $this->storage->findAddress('г москва, ул стахановская')
        );

        $this->assertNull($this->storage->findAddress('Ерунда какая-то, а не адрес.'));
    }
    ...
    public function testFindHouse()
    {
        $this->assertEquals(
            ...
            $this->storage->findHouse('г москва, ул стахановская, 16с17')
        );
